...
public class EquityRecord extends OrgaDeptAct {
    ...
    @TableId("record_id")
    @Comment("记录编号")
    @Pattern(regexp = "[0-9]{18}",message = "全怒i兑换记录编号必须位18位数字")
...
//    private String orgaUserId;
    ...
    @Comment("记录类型  增加 | 减少")
    @InRange(clazz = EquityReType.class,message = "记录类型必须是在EquityReType中声明的值")
    private String recordType;

    @Comment("详细类型 增加渠道 | 减少渠道 理由")
    @InRange(clazz = EquityReDetailType.class,message = "权益记录详细类型必须是在EquityReDetailType中声明的值")
